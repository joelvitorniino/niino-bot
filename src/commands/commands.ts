...
export async function start(client: Client) {
  ...
  // });
    ...
    await client.onMessage(async (message: Message) => {
        const niino = await Niino.findAll({
        where: {
        ...
        const regex = new RegExp('(desisto|desistir|desistindo|desisti|recaindo|recaída|recaida|jogar no vasco|Jogar no Vasco|quero parar tudo| quero jogar tudo pro alto)');
        const regexCharcoal = new RegExp('(carvão|carvao)');
        
        if(regex.test(message.body)) {
          await client.sendText(message.from, 'Não desista amigo, confie no seu potencial!');
        };
        ...
        if(groupId.includes(`${message.chatId}`) && command.includes(`${message.body.split(" ").shift()}`)) {
          ...
          if(validCommandRegex.test(msgBody) || validCommandCaptionRegex.test(msgBody)) {
            try {
              ...
              if(message.isGroupMsg === false) {
                await client.sendText(message.from, 'Olá este comando não existe ou está escrito errado!');
              }
            }
          };
        };
